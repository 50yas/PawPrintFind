export const en = {
    // UI Elements
    theme: `Theme`,
    lightMode: `Light`,
    darkMode: `Dark`,
    systemMode: `System`,
    homeButton: `Home`,
    blogButton: `Blog`,
    ecosystemButton: `Ecosystem`,
    supportButton: `Support Us`,
    homeAriaLabel: `Go to Homepage`,
    loginButton: `Login`,
    logoutButton: `Logout`,
    dashboardButton: `Dashboard`,
    myImpronteButton: `My Impronte`,
    cancelButton: `Cancel`,
    closeButton: `Close`,
    improntaSavedAlert: `{{petName}}'s Impronta has been saved.`,
    footerText: `Paw Print - Your pet's unique digital imprint to find them faster.`,
    openSourceFooter: `Proudly Open Source. Join our community on GitHub!`,
    vetRegisteredAlert: `Thank you! {{clinicName}} has been registered as a partner clinic.`,
    
    // Hero Section - Adjusted for better flow
    homeTitle1: `Every PawPrint`,
    homeTitle2: `is Unique`, 
    homeSubtitle: `The fastest way to reunite with your lost pet. Create a secure digital passport today.`,
    createImprontaButton: `Create Impronta 🐾`,
    foundPetButton: `I Found a Pet 🐱`,
    explorerMapButton: `Explorer Map`,
    aiScannerButton: `AI Scanner`,
    
    // Ecosystem
    howItWorksTitle: `Ecosystem Explorer`,
    ecosystemDesc: `Paw Print connects owners, finders, and vets in a seamless network. Choose your role to see how we empower you.`,
    roleOwner: `Pet Owner`,
    roleFinder: `Pet Finder`,
    roleVet: `Veterinarian`,
    
    roleOwnerStep1Title: `Create Digital ID`,
    roleOwnerStep1Desc: `Build a comprehensive biometric profile with photos, marks, and medical data.`,
    roleOwnerStep2Title: `Smart Alert System`,
    roleOwnerStep2Desc: `Instantly notify the community and vets within a 5km radius if your pet goes missing.`,
    roleOwnerStep3Title: `Secure Reunion`,
    roleOwnerStep3Desc: `Verify ownership through AI matching and coordinate safe pickups via secure chat.`,
    roleFinderStep1Title: `Snap & Scan`,
    roleFinderStep1Desc: `Found a stray? Take a photo. Our AI analyzes visual features to find a match.`,
    roleFinderStep2Title: `AI Verification`,
    roleFinderStep2Desc: `Compare unique markings against our lost pet database in seconds.`,
    roleFinderStep3Title: `Connect Safely`,
    roleFinderStep3Desc: `Message the owner directly without revealing your phone number or address.`,
    roleVetStep1Title: `Patient Management`,
    roleVetStep1Desc: `Access digital health records and manage appointments for your furry patients.`,
    roleVetStep2Title: `AI Diagnostics`,
    roleVetStep2Desc: `Use our AI Health Assistant to triage symptoms and draft care instructions.`,
    roleVetStep3Title: `Community Hub`,
    roleVetStep3Desc: `Become a registered safe haven for lost pets and grow your practice visibility.`,
    forVetsTitle: `For Veterinarians`,
    forVetsDesc: `Join our network of heroes. Help reunite lost pets with their families and promote your clinic to local pet owners.`,
    registerClinicButton: `Register Your Clinic`,
    openSourceTitle: `A Community Effort`,
    openSourceDesc: `Paw Print is an open-source project. We believe collaboration is the best way to help more animals.`,
    poweredByGemini: `Powered by Gemini AI`,
    liveAssistantTitle: `Live AI Assistant`,
    createImprontaTitle: `Create Your Pet's Impronta`,
    createImprontaDescription: `This information creates a unique visual ID to help identify your pet if they get lost.`,
    step1: `Basic Info`,
    step2: `Photos & Marks`,
    step3: `Advanced Profile`,
    step4: `Location & Save`,
    petNameLabel: `Pet's Name`,
    breedLabel: `Breed`,
    ageLabel: `Age`,
    weightLabel: `Weight`,
    behaviorLabel: `Behavioral Notes`,
    uploadPhotoButton: `Upload Photo`,
    tagUniqueMarks: `Tag Unique Marks`,
    tagUniqueMarksDesc: `Click on the photos to add markers for unique features.`,
    backButton: `Back`,
    nextButton: `Next`,
    submitButton: `Save Impronta`,
    statusReady: `Ready to start.`,
    statusConnecting: `Connecting...`,
    statusConnected: `Connected. Speak now.`,
    statusError: `Error`,
    statusConnectionClosed: `Connection closed.`,
    liveAssistantWelcomeTitle: `Welcome to the Live Assistant!`,
    liveAssistantWelcomeDesc: `I'm here to help you identify a found pet or manage your profile.`,
    typeMessagePlaceholder: `Type a message...`,
    dashboardTitle: `My Impronte`,
    reportLostButton: `Report as Lost`,
    markFoundButton: `Mark as Found`,
    editButton: `Edit`,
    missingPetsMapTitle: `Missing Pets Map`,
    missingPetsMapDesc: `Recently reported lost pets in our community.`,
    askAiButtonLabel: `Ask AI`,
    scanning: `SCANNING...`,
    analyzing: `ANALYZING...`,
    identified: `IDENTIFIED`,
    visualIdLocked: `VISUAL ID LOCKED`,
    findVetTitle: `Find a Veterinarian`,
    findVetDesc: `Search for a vet by location.`,
    supportUsTitle: `Support Us`,
    supportUsDesc: `Your support helps us keep the servers running.`,
    donorsButton: `Our Donors`,
    donorsPageTitle: `Community Heroes`,
    aiPoweredProtection: `AI-Powered Pet Protection`,

    // --- New Translations ---
    adoptionInquiriesTitle: `Adoption Inquiries`,
    inquiryAbout: `Inquiry about {{petName}}`,
    
    // Auth
    placeholders: {
        email: `Enter Secure Email`,
        password: `Enter Access Key`,
        confirmPassword: `Confirm Access Key`,
        phoneNumber: `Phone Number (for Alerts)`,
    },
    title: {
        login: `System Login`,
        register: `New Identity`,
        recovery: `Key Recovery`,
        magicLink: `Magic Link`,
    },
    subtitle: {
        login: `Enter your credentials to access the Paw Print network.`,
        register: `Create a secure biometric profile for your pet.`,
        recovery: `Reset your access key via secure email link.`,
        magicLink: `We will send a secure access link to your email.`,
    },
    buttons: {
        decryptDashboard: `Decrypt Dashboard`,
        authorizeNewAccount: `Authorize Identity`,
        initiateRecovery: `Initiate Recovery`,
        sendMagicLink: `Send Magic Link`,
        alreadyRegistered: `Already registered?`,
        signInProtocol: `Sign In Protocol`,
        newOperative: `New Operative?`,
        initializeProfile: `Initialize Profile`,
        lostCredentials: `Lost Credentials?`,
        backToIdentification: `Back to Identification`,
        syncGoogle: `Sync with Google`,
        useMagicLink: `Use Magic Link`,
        usePassword: `Use Password`,
    },
    errors: {
        emailInUse: `This email is already registered in the database.`,
        invalidCredential: `Invalid credentials. Access denied.`,
        userNotFound: `User identity not found in the system.`,
        wrongPassword: `Incorrect access key. Access denied.`,
        invalidEmail: `Invalid email format detected.`,
        popupClosed: `Authentication window closed by user.`,
        weakPassword: `Password is too weak. Enhance complexity.`,
        passwordMismatch: `Access keys do not match.`,
        requiredFields: `All fields are required for authorization.`,
        missingEmail: `Email is required for Magic Link.`,
        default: `An unknown error occurred.`,
    },
    success: {
        recoverySent: `Recovery instructions sent to secure email.`,
        accountCreated: `Identity authorized. Welcome to the network.`,
        magicLinkSent: `Magic link sent! Check your inbox to access the system.`,
    },
    
    // Admin Professional Mgmt
    infrastructureTitle: `Veterinary Infrastructure`,
    newVetButton: `New Vet Identity`,
    newClinicButton: `New Clinic Node`,
    authorizedFacilities: `Authorized Facilities`,
    pendingInfrastructure: `Vets Pending Infrastructure Link`,
    verifiedPro: `Verified Pro`,
    pendingVerification: `Pending Verification`,
    linkedStatus: `Linked`,
    unlinkedStatus: `Unlinked`,
    createClinicAction: `Create Clinic`,
    verifyNowAction: `Verify Now`,
    dismantleButton: `Dismantle`,
    purgeButton: `Purge`,
    terminateButton: `Terminate`,
    
    // Filters
    allRoles: `All Roles`,
    allStatus: `All Status`,
    verifiedOnly: `Verified Only`,
    unverifiedOnly: `Unverified Only`,
    statusLost: `Lost`,
    statusAdoption: `Adoption`,
    statusOwned: `Owned`,
    
    // Map Filters
    showLostPets: `Lost Pets`,
    showAdoptablePets: `Adoption`,
    
    // Navbar & Adoption Center
    adoptionLink: `Adoption`,
    adoptionCenterTitle: `Adoption Center`,
    adoptionCenterDesc: `Find your new best friend.`,
    adoptionCenterDescPublic: `Browse pets available for adoption from our partner shelters.`,
    inquireToAdoptButton: `Inquire to Adopt`,
    noPetsForAdoption: `No pets currently available for adoption.`,
    loginToAdoptWarning: `You must be logged in to inquire about adoption.`,
    genderLabel: `Gender`,
    genderMale: `Male`,
    genderFemale: `Female`,
    genderUnknown: `Unknown`,
    filterByBreedLabel: `Filter by Breed`,
    filterAgeLabel: `Age`,
    adoptionMapTitle: `Adoption Map`,
    locateMeButton: `Locate Me`,
    adoptMeButton: `Adopt Me`,
    adoptionInquiryTemplate: `Hello! I am very interested in adopting {{petName}}. Could you provide more information about the adoption process?`,
    loginToContactOwner: `Please login to contact the owner.`,
    contactOwnerButton: `Contact Owner`,
    
    // Tutorial
    tutorial: {
        step: `Step`,
        skip: `Skip Tutorial`,
        finish: `Finish`,
        home: {
            title: `Welcome to Paw Print`,
            desc: `This is your central hub. From here, you can access all main features of the app.`
        },
        register: {
            title: `Create an Impronta`,
            desc: `Register your pet's biometric profile here. It's the best way to keep them safe.`
        },
        map: {
            title: `Live Map`,
            desc: `See lost pets and community alerts in real-time on our interactive map.`
        },
        community: {
            title: `Community Hub`,
            desc: `Connect with other pet owners, join the leaderboard, and help find lost pets.`
        },
        emergency: {
            title: `AI Assistant`,
            desc: `Need help fast? Tap here to speak with our AI Assistant for immediate guidance.`
        }
    },

    // AI Health Analytics
    aiAnalyticsDashboardTitle: `AI Health Analytics`,
    aiAnalyticsSubtitle: `Aggregate health insights across your practice.`,
    averageHealthScore: `Average Health Score`,
    totalPatientsAnalyzed: `Patients Analyzed`,
    generateReportButton: `Generate Report`,
    recentInsightsTitle: `Recent Insights`,
    noAnalyticsData: `No Health Data Available`,
    noAnalyticsDataDesc: `Add medical records or run AI health checks on your patients to see insights here.`,

    // Priority Support
    prioritySupportTitle: `Priority Support`,
    prioritySupportDesc: `Direct Line for Pro Vets`,
    prioritySupportSubtext: `Need technical assistance or have a feature request? Connect directly with our engineering team.`,
    startLiveChatButton: `Start Live Chat`,
    sendEmailButton: `Send Urgent Email`,
    chatConnecting: `Connecting to Agent...`,
    avgResponseTime: `Avg. Response Time`,

    // My Clinic
    myClinicTitle: `Manage Your Clinic`,
    myClinicDesc: `Keep your practice details up-to-date to help lost pets find their way to you. Sync with Google Maps for instant accuracy.`,
    searchClinicPlaceholder: `Search for your clinic on Google Maps...`,
    searchButtonGoogle: `Search Maps`,
    clinicNameLabel: `Clinic Name`,
    addressLabel: `Address`,
    phoneLabel: `Phone Number`,
    saveClinicButton: `Save Changes`,
    searchToSyncTitle: `Sync from Google Maps`,
    searchToSyncDesc: `Search for your clinic on the left to instantly populate your details.`,
    syncAllDataButton: `Sync Data`,
    noGoogleMapsVets: `No clinics found with that name.`,
    addressNotFound: `Address not found in search result.`,
    clinicSaved: `Clinic details saved successfully!`,
    
    verifiedVetBadge: `Verified Vet`,

    // Added keys
    loginToSaveSearchWarning: `You must be logged in to save a search.`,

    dashboard: {
        admin: {
            systemRootActive: `SYSTEM_ROOT_ACTIVE - Full platform administrative privileges authorized.`,
            contentIntelligence: `Content Intelligence - Analyzing community engagement and blog performance metrics.`,
            rankAlpha: `Rank: ALPHA (Tier 1 Performance)`,
            rankBeta: `Rank: BETA (Tier 2 Performance)`,
            rankGamma: `Rank: GAMMA (Tier 3 Performance)`,
            zeroViews: `0 VIEWS - No engagement detected yet.`,
            viewsLabel: `VIEWS - Total unique identity hits recorded.`,
            totalEngagement: `Total Engagement - Aggregated user interaction depth.`,
            adminTabDonations: `Finance & Donations`,
            adminTabI18n: `Global Localization`,
            adminTabSocial: `Social Discovery`,
            adminTabGamification: `Gamification & XP`,
            adminTabConfig: `System Config`,
            totalRevenue: `Total Revenue - Aggregate capital contribution to platform operations.`,
            donor: `Donor Entity`,
            amount: `Amount`,
            date: `Timestamp`,
            anonymousDonor: `Anonymous Entity`,
            status_paid: `PAID - Transaction finalized in ledger.`,
            status_pending_payment: `PENDING - Waiting for network confirmation.`,
            status_failed: `FAILED - Transaction protocol aborted.`,
            confirmPurgeDonation: `WARNING: This will permanently purge this donation record from the ledger. Continue?`,
            donationPurged: `Donation record successfully purged from history.`,
            confirmPurgeContent: `WARNING: This will permanently delete this content entry. This action cannot be reversed.`,
            blogAccessDenied: `Access to blog repository denied. Verify clearance level.`,
            nodeSynchronized: `Local node synchronized with global cloud vault.`,
            refreshFailed: `Synchronization protocol failed: `,
            confirmTerminateIdentity: `WARNING: This will permanently terminate this user identity and all associated data.`,
            identityPurged: `User identity purged from system registry.`,
            clearanceUpdated: `User clearance level and roles updated successfully.`,
            confirmDismantleClinic: `WARNING: This will permanently dismantle this clinic node. Active patients may lose local link.`,
            clinicDismantled: `Clinic node dismantled successfully.`,
            approvedCredentials: `Approved professional credentials for {{email}}.`,
            rejectVerificationPrompt: `Reject credentials for {{email}}? Access to professional tools will be revoked.`,
            verificationRejected: `Verification protocol rejected. User notified.`,
            commandCore: `COMMAND_CORE`,
            urgentProtocol: `URGENT_PROTOCOL`,
            resolveNow: `RESOLVE_NOW`,
            loadFactor: `LOAD_FACTOR - Real-time system stress level analysis.`,
            syncNode: `SYNC_NODE - Force refresh of local registry data.`,
            exitSession: `EXIT_SESSION`,
            adminTabOverview: `Operations Overview`,
            adminTabUsers: `Identity Registry`,
            adminTabClinics: `Medical Nodes`,
            adminTabPets: `Biometric Registry`,
            adminTabBlog: `Content Hub`,
            optimizeTab: `Neural Tuning`,
            adminTabLogs: `System Logs`,
            pendingVerificationsTitle: `Identity Verification`,
            pendingVerificationsDesc: `Manual audit required for professional medical credentials.`,
            views: `Views`,
            contentHeader: `Content Metadata`,
            author: `Author Identity`,
            analytics: `Metric Analysis`,
            actionPool: `Action Pool`,
            viewButton: `View Protocol`,
            editButton: `Edit Record`,
            deleteUserButton: `Purge Record`,
            viewDocs: `Audit Documents`,
            acceptButton: `Authorize`,
            declineButton: `Decline`,
            noPendingSequences: `No pending verification sequences detected in the queue.`,
            flushMemory: `FLUSH_MEMORY - Clear local session cache.`,
            searchPetPlaceholder: `Search Biometric Registry (Name, ID, Breed)...`,
            allStatus: `ALL_STATUS`,
            statusLost: `LOST - Currently untracked in the wild.`,
            statusAdoption: `ADOPTION - Open for new identity linkage.`,
            statusOwned: `OWNED - Linked to valid user identity.`,
            orbitalUnknown: `ORBITAL_UNKNOWN - Position data currently unavailable.`,
            terminateButton: `TERMINATE`,
            confirmTerminateProfile: `TERMINATE PET PROFILE? All biometric data will be lost.`,
            blogRepository: `CONTENT_REPOSITORY`,
            newEntry: `NEW_ENTRY`,
            clinicNameLabel: `Facility Designation`,
            contactTitle: `Comms Link`,
            addressLabel: `Orbital Address`,
            tableActions: `Ops Pool`,
            verifiedStatus: `VERIFIED`,
            dismantleButton: `DISMANTLE`,
            noVetsFound: `NO_MEDICAL_NODES_DETECTED`,
            pendingInfrastructure: `PENDING_INFRASTRUCTURE_LINK`,
            tableRole: `IDENTITY_TYPE`,
            status: `PROTOCOL_STATUS`,
            verifiedPro: `VERIFIED_OPERATIVE`,
            pendingVerification: `PENDING_AUDIT`,
            linkedStatus: `LINKED_TO_FACILITY`,
            unlinkedStatus: `UNLINKED_NODES`,
            createClinicAction: `PROVISION_NODE`,
            verifyNowAction: `AUDIT_NOW`,
            noEngagementData: `NO_ENGAGEMENT_TELEMETRY_DETECTED`,
            petUpdated: `Pet biometric record updated successfully.`,
            configUpdated: `System configuration updated across all nodes.`
        },
        adoption: {
             matchesFound: `{{count}} matches found`,
             noMatchingPets: `No matching pets found`,
             adjustFiltersSuggestion: `Try adjusting your filters to see more adoptable friends.`,
             clearFiltersButton: `Clear all filters`
        }
    }
};