...
const WordmarkSVG = ({ fill = "white" }: { fill?: string }) => (
    <svg viewBox="0 0 400 100" xmlns="http://www.w3.org/2000/svg">
        <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" fontFamily="'Plus Jakarta Sans', sans-serif" fontWeight="800" fontSize="60" fill={fill}>
            Paw Print
        </text>
    </svg>
);
...
const LockupSVG = ({ fill = "white" }: { fill?: string }) => (
    <svg viewBox="0 0 600 200" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="220" y="110" fontFamily="'Plus Jakarta Sans', sans-serif" fontWeight="800" fontSize="80" fill={fill}>Paw Print</text>
    </svg>
);
    ...
    const getSvgString = (fillColor: string) => {
        const svgContent = 
            type === 'icon' ? (
                ...
                `<svg viewBox="0 0 400 100" xmlns="http://www.w3.org/2000/svg"><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="sans-serif" font-weight="800" font-size="60" fill="${fillColor}">Paw Print</text></svg>`
            ) : (
                `<svg viewBox="0 0 600 200" xmlns="http://www.w3.org/2000/svg"><g transform="translate(50, 20) scale(0.3)"><path fill="${fillColor}" d="M490.39,182.75c-5.55-13.19-14.77-22.7-26.67-27.49l-.16-.06a46.46,46.46,0,0,0-17-3.2h-.64c-27.24.41-55.05,23.56-69.19,57.61-10.37,24.9-11.56,51.68-3.18,71.64,5.54,13.2,14.78,22.71,26.73,27.5l.13.05a46.53,46.53,0,0,0,17,3.2c27.5,0,55.6-23.15,70-57.65C497.65,229.48,498.78,202.72,490.39,182.75Z"/><path fill="${fillColor}" d="M381.55,329.61c-15.71-9.44-30.56-18.37-40.26-34.41C314.53,250.8,298.37,224,256,224s-58.57,26.8-85.39,71.2c-9.72,16.06-24.6,25-40.36,34.48-18.07,10.86-36.74,22.08-44.8,44.16a66.93,66.93,0,0,0-4.65,25c0,35.95,28,65.2,62.4,65.2,17.75,0,36.64-6.15,56.63-12.66,19.22-6.26,39.09-12.73,56.27-12.73s37,6.47,56.15,12.73C332.2,457.85,351,464,368.8,464c34.35,0,62.3-29.25,62.3-65.2a67,67,0,0,0-4.75-25C418.29,351.7,399.61,340.47,381.55,329.61Z"/><path fill="${fillColor}" d="M150,188.85c11.9,14.93,27,23.15,42.52,23.15a42.88,42.88,0,0,0,6.33-.47c32.37-4.76,52.54-44.26,45.92-90C242,102.3,234.6,84.39,224,71.11,212.12,56.21,197,48,181.49,48a42.88,42.88,0,0,0-6.33.47c-32.37,4.76-52.54,44.26-45.92,90C132,157.67,139.4,175.56,150,188.85Z"/><path fill="${fillColor}" d="M313.16,211.53a42.88,42.88,0,0,0,6.33.47c15.53,0,30.62-8.22,42.52-23.15,10.59-13.29,17.95-31.18,20.75-50.4h0c6.62-45.72-13.55-85.22-45.92-90a42.88,42.88,0,0,0-6.33-.47C315,48,299.88,56.21,288,71.11c-10.6,13.28-18,31.19-20.76,50.44C260.62,167.27,280.79,206.77,313.16,211.53Z"/><path fill="${fillColor}" d="M111.59,308.8l.14-.05c11.93-4.79,21.16-14.29,26.69-27.48,8.38-20,7.2-46.75-3.15-71.65C120.94,175.16,92.85,152,65.38,152a46.4,46.4,0,0,0-17,3.2l-.14.05C36.34,160,27.11,169.54,21.58,182.73c-8.38,20-7.2,46.75,3.15,71.65C39.06,288.84,67.15,312,94.62,312A46.4,46.4,0,0,0,111.59,308.8Z"/></g><text x="220" y="110" font-family="sans-serif" font-weight="800" font-size="80" fill="${fillColor}">Paw Print</text></svg>`
            );
        return svgContent;
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={t('downloadAsset')}>
            <div className="space-y-6">
                {/* Preview */}
                <div className={`w-full aspect-[2/1] rounded-2xl flex items-center justify-center border-2 border-dashed border-border ${background === 'colored' ? 'bg-primary' : 'bg-grid-pattern'}`}>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        <label className="text-xs font-bold text-muted-foreground uppercase mb-2 block">{t('formatLabel')}</label>
                        <div className="flex gap-2">
                            {(['png', 'jpg', 'svg'] as FileFormat[]).map(f => (
                                <button 
                                    key={f} 
                                    onClick={() => setFormat(f)}
                                    className={`px-3 py-2 rounded-lg text-sm font-bold border transition-all ${format === f ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:border-primary'}`}
                                >
                                    {f.toUpperCase()}
                                </button>
                            ))}
                    ...
                    <div>
                        <label className="text-xs font-bold text-muted-foreground uppercase mb-2 block">{t('bgLabel')}</label>
                        <div className="flex gap-2">
                            <button 
                                onClick={() => setBackground('transparent')}
                                className={`px-3 py-2 rounded-lg text-sm font-bold border transition-all ${background === 'transparent' ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:border-primary'}`}
                            >
                                {t('transparentBg')}
                            </button>
                            <button 
                                onClick={() => setBackground('colored')}
                                className={`px-3 py-2 rounded-lg text-sm font-bold border transition-all ${background === 'colored' ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:border-primary'}`}
                            >
                                {t('coloredBg')}
                            </button>
                        </div>
                ...
                <div className="pt-4 border-t border-border">
                    <button onClick={handleDownload} className="w-full btn btn-primary">
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12 pb-24">
            ...
            <header className="text-center space-y-4">
                <h1 className="text-4xl md:text-5xl font-extrabold text-foreground tracking-tight">{t('pressKitTitle')}</h1>
                <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{t('pressKitDesc')}</p>
            </header>

            {/* Asset Grid */}
            <div className="grid md:grid-cols-3 gap-8">
                {/* 1. Icon Only */}
                <div onClick={() => setSelectedAsset('icon')} className="bg-card hover:bg-muted/50 border border-border rounded-3xl p-8 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 group">
                    <div className="h-40 flex items-center justify-center mb-6 bg-muted/20 rounded-2xl border border-dashed border-border group-hover:border-primary/30">
                        <div className="w-24 h-24 text-primary">
                            <IconSVG fill="currentColor" />
                        </div>
                    </div>
                    <div className="text-center">
                        <h3 className="text-xl font-bold text-foreground">{t('assetLogoIcon')}</h3>
                        <p className="text-sm text-muted-foreground mt-2">Best for avatars, favicons, and app icons.</p>
                    </div>
                </div>

                {/* 2. Wordmark Only */}
                <div onClick={() => setSelectedAsset('wordmark')} className="bg-card hover:bg-muted/50 border border-border rounded-3xl p-8 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 group">
                    <div className="h-40 flex items-center justify-center mb-6 bg-muted/20 rounded-2xl border border-dashed border-border group-hover:border-primary/30">
                        <div className="w-48 text-primary">
                            <WordmarkSVG fill="currentColor" />
                        </div>
                    </div>
                    <div className="text-center">
                        <h3 className="text-xl font-bold text-foreground">{t('assetLogoText')}</h3>
                        <p className="text-sm text-muted-foreground mt-2">The brand name in official typography.</p>
                    </div>
                </div>

                {/* 3. Full Lockup */}
                <div onClick={() => setSelectedAsset('lockup')} className="bg-card hover:bg-muted/50 border border-border rounded-3xl p-8 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 group">
                    <div className="h-40 flex items-center justify-center mb-6 bg-muted/20 rounded-2xl border border-dashed border-border group-hover:border-primary/30">
                    ...
                    <div className="text-center">
                        <h3 className="text-xl font-bold text-foreground">{t('assetLogoLockup')}</h3>
                        <p className="text-sm text-muted-foreground mt-2">The standard logo for headers and partnerships.</p>
                    </div>
                </div>
            </div>
            ...
            <section className="bg-card border border-border rounded-3xl p-8 shadow-sm">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    <div>
                        <p className="text-sm text-muted-foreground mb-2 font-mono">UI Font</p>
                        <h3 className="text-4xl font-extrabold mb-2 font-sans">Plus Jakarta Sans</h3>
                        <p className="text-lg">Aa Bb Cc Dd Ee Ff Gg 123</p>
                    </div>
                    <div>
                        <p className="text-sm text-muted-foreground mb-2 font-mono">Data/Tech Font</p>
                        <h3 className="text-4xl font-mono-tech mb-2">Space Mono</h3>
                        <p className="text-lg font-mono-tech">Aa Bb Cc Dd Ee Ff Gg 123</p>
                    </div>
                </div>
            </section>
        </div>
