...
export const MissingPetsMap: React.FC<MissingPetsMapProps> = ({ lostPets, vetClinics = [], userLocation, isPostingMode, onMapClick }) => {
  ...
  return (
    <div className="relative w-full h-full bg-slate-100 overflow-hidden rounded-2xl border border-border">
        {/* MAP HUD CONTROLS */}
        <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-2">
            <div className="glass-panel p-1 rounded-xl flex shadow-xl border-white/20">
                <button 
                    onClick={() => switchStyle('street')}
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${mapStyle === 'street' ? 'bg-primary text-white shadow-lg' : 'text-muted-foreground hover:bg-white/10'}`}
                >Street</button>
                <button 
                    onClick={() => switchStyle('satellite')}
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${mapStyle === 'satellite' ? 'bg-primary text-white shadow-lg' : 'text-muted-foreground hover:bg-white/10'}`}
                >Satellite</button>
            </div>
            
            {vetClinics.length > 0 && (
                <div className="glass-panel p-2 px-3 rounded-xl shadow-lg border-white/20 text-center animate-fade-in">
                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-tighter">
                        {vetClinics.filter(v=>v.isVerified).length} Verified Vets Nearby
                    </p>
                </div>
            )}
        </div>
