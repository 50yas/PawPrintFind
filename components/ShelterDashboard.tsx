    ...
    <div className="max-w-5xl mx-auto space-y-12 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-purple-900 to-indigo-900 p-8 rounded-3xl text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
             <h1 className="text-3xl font-bold">{t('shelterDashboardTitle')}</h1>
             <p className="text-purple-100 mt-1">Manage adoptions and inquiries</p>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {shelterPets.length > 0 ? (
              ...
              <div className="bg-muted/30 p-12 rounded-3xl text-center border-2 border-dashed border-border flex flex-col items-center">
                ...
                <h3 className="text-xl font-bold text-foreground">{t('noAnimalsRegistered')}</h3>
                <p className="text-muted-foreground mt-2 mb-6 max-w-md">{t('noAnimalsRegisteredDesc')}</p>
                <button onClick={onRegisterNew} className="btn btn-primary">{t('registerFirstAnimalButton')}</button>
              </div>
            )}
          </div>
                          ...
                          {chatSessions.map(chat => (
                              <div key={chat.id} onClick={() => onOpenChat(chat.id)} className="p-4 hover:bg-muted/50 cursor-pointer transition-colors flex items-start gap-3 group">
                                  ...
                                  <div className="flex-grow min-w-0">
                                      ...
                                      <p className="text-xs text-primary mt-2 font-medium group-hover:underline">View Conversation &rarr;</p>
                                  </div>
                              </div>
                          ))}
                      </div>
