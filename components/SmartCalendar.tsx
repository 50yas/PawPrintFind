    ...
    <div className="max-w-6xl mx-auto grid lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-4 space-y-8">
         ...
         <div className="bg-card p-6 rounded-2xl shadow-md border border-border">
          <h2 className="text-xl font-bold text-card-foreground border-b border-border pb-4 mb-4">{t('newAppointmentTitle')}</h2>
      ...
      <div className="lg:col-span-8 bg-card p-8 rounded-2xl shadow-lg border border-border h-full flex flex-col">
        <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-card-foreground">{t('upcomingAppointments')}</h3>
        ...
        <div className="flex-grow space-y-1 overflow-y-auto pr-2">
            {upcomingAppointments.length > 0 ? (
                upcomingAppointments.map((app, index) => {
                     ...
                     return (
                        <div key={app.id} className="flex gap-4 py-3 group hover:bg-muted/50 rounded-xl p-2 transition-colors">
                            <div className="flex flex-col items-center min-w-[60px]">
                                <span className="text-xs font-bold text-muted-foreground uppercase">{appDate.toLocaleDateString(undefined, { month: 'short' })}</span>
                                <span className={`text-2xl font-bold ${isToday ? 'text-primary' : 'text-foreground'}`}>{appDate.getDate()}</span>
                            </div>
                            <div className="w-1.5 rounded-full bg-gray-200 dark:bg-gray-700 relative">
                                {isToday && <div className="absolute top-0 w-full h-full bg-primary rounded-full"></div>}
                            </div>
                            <div className="flex-grow pb-4 border-b border-border/50 group-last:border-0">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h4 className="text-lg font-bold text-foreground">{app.petName}</h4>
                                        <p className="text-sm text-primary font-medium">{app.time}</p>
                                        <p className="text-sm text-muted-foreground mt-1">{app.notes || t('generalCheckup')}</p>
                                    </div>
                                    {app.status === 'pending' ? (
                                        <div className="flex flex-col gap-2">
                                            <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-[10px] font-bold uppercase tracking-wide rounded-full self-end">{t('pendingStatusBadge')}</span>
                                            <div className="flex gap-2">
                                                <button onClick={() => onStatusChange(app.id, 'confirmed')} className="p-1 bg-green-100 text-green-600 rounded-full hover:bg-green-200" title={t('confirmAction')}><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg></button>
                                                <button onClick={() => onStatusChange(app.id, 'cancelled')} className="p-1 bg-red-100 text-red-600 rounded-full hover:bg-red-200" title={t('declineAction')}><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button>
                                            </div>
                                        </div>
                                    ) : (
                                        <span className="px-2 py-1 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 text-[10px] font-bold uppercase tracking-wide rounded-full">{t('confirmedStatus')}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center opacity-60">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    <p className="text-lg font-medium text-foreground">{t('noAppointments')}</p>
