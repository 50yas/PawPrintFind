                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {DONATION_TIERS.map((tier) => (
                            <button
                                ...
                                className={`relative p-3 rounded-xl border-2 transition-all duration-200 flex flex-col items-center justify-center text-center group ${
                                    !isCustom && selectedAmount === tier.amount 
                                    ? 'border-primary bg-primary/5 shadow-md scale-105 z-10' 
                                    : 'border-border bg-card hover:border-primary/50'
                                }`}
                            >
                                <div className="text-2xl mb-1 group-hover:scale-110 transition-transform">{tier.emoji}</div>
                                <div className="font-bold text-lg text-foreground">€{tier.amount}</div>
                                <div className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold mt-1">{tier.label}</div>
                            </button>
                        ))}
                    </div>
