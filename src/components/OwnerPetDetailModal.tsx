    ...
    <Modal isOpen={true} onClose={onClose} title={pet.name}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 backdrop-blur-xl p-4 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center">
                <span className="text-xs text-slate-400 uppercase tracking-wider">{t('weightLabel')}</span>
                <span className="text-lg font-bold text-white">{pet.weight || 'N/A'}</span>
            </div>
            <div className="bg-white/5 backdrop-blur-xl p-4 rounded-xl border border-white/10 flex flex-col items-center justify-center text-center">
                <span className="text-xs text-slate-400 uppercase tracking-wider">{t('vetLinkedBadge')}</span>
        ...
        <div className="bg-indigo-900/20 p-4 rounded-xl border border-indigo-800">
            ...
            <div className="text-sm space-y-1 text-indigo-300">
        ...
        <div className="bg-white/5 backdrop-blur-xl p-4 rounded-xl border border-white/10 shadow-sm">
            ...
            {petAppointments.length > 0 ? (
                <div className="space-y-2">
                    {petAppointments.map(app => (
                        <div key={app.id} className="flex items-center gap-3 p-2 bg-white/5 rounded-lg">
                            ...
                            <div className="flex-grow">
                                <p className="text-sm font-semibold text-white">{app.notes || 'Check-up'}</p>
                ...
                <p className="text-sm text-slate-400 text-center py-4 italic">{t('noAppointments')}</p>
            )}
        </div>
