...
describe('VetPremiumModal Component', () => {
  ...
  it('renders correctly when open', () => {
    render(
      <SnackbarProvider>
        <VetPremiumModal {...defaultProps} />
      </SnackbarProvider>
    );
    expect(screen.getByText(/upgradeTitle/i)).toBeInTheDocument();
    expect(screen.getByText(/vetPro/i)).toBeInTheDocument();
  ...
  it('calls subscribeToPlan when Upgrade Now is clicked', async () => {
    ...
    const upgradeBtn = screen.getByText(/upgradeNow/i);
  ...
  it('does not render when closed', () => {
    render(
      <SnackbarProvider>
        <VetPremiumModal {...defaultProps} isOpen={false} />
      </SnackbarProvider>
    );
