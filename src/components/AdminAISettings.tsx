...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    blogGeneration: { icon: '✍️', color: 'text-pink-400', borderColor: 'border-l-pink-500' },
};
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini ? 'ENABLED' : 'DISABLED', icon: '🔄', glow: settings.fallbackToGemini ? 'neon-glow-blue' : '' },
                ].map((stat, i) => (
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider cursor-pointer" htmlFor="fallback-toggle">
                            {t('dashboard:admin.fallbackToGemini')}
                        </label>
                        <button
                            id="fallback-toggle"
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-10 h-5 rounded-full relative transition-all duration-300 ${settings.fallbackToGemini ? 'bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.4)]' : 'bg-slate-700'}`}
                        >
                            <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all duration-300 ${settings.fallbackToGemini ? 'left-6' : 'left-1'}`}></div>
                        </button>
                    </div>
                </div>
                                    ...
                                    <datalist id={`models-${task.id}`}>
                                        {settings.provider === 'google' ? (
                                            ...
                                            <>
                                                ...
                                                <option value="meta-llama/llama-3.1-405b-instruct:free">llama-3.1-405b (Super Intelligent)</option>
