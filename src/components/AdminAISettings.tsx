                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black text-white uppercase tracking-tighter">Fallback to Gemini</span>
                            <span className="text-[7px] text-slate-500 uppercase">Auto-retry on OpenRouter fail</span>
                        </div>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${settings.fallbackToGemini ? 'bg-blue-500' : 'bg-slate-700'}`}
                        >
