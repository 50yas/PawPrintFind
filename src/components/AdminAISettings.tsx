...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    healthAssessment: { icon: '🏥', color: 'text-red-400', borderColor: 'border-l-red-500' },
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini ? 'ENABLED' : 'DISABLED', icon: '🛡️', glow: settings.fallbackToGemini ? 'neon-glow-teal' : 'neon-glow-red' },
                ].map((stat, i) => (
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{t('dashboard:admin.fallbackToGemini')}</span>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`relative w-10 h-5 rounded-full transition-colors duration-300 ${settings.fallbackToGemini ? 'bg-primary' : 'bg-slate-700'}`}
                        >
                            <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform duration-300 ${settings.fallbackToGemini ? 'translate-x-5' : ''}`}></div>
                        </button>
                    </div>
                </div>
