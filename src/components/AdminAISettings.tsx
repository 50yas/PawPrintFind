        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini ? 'READY (GEMINI)' : 'DISABLED', icon: '🛡️', glow: settings.fallbackToGemini ? 'neon-glow-amber' : '' },
                    ...
                    <div key={i} className={`p-4 rounded-2xl bg-white/5 border border-white/10 text-center transition-all duration-300 hover:bg-white/10 ${stat.glow}`}>
                        ...
                        <p className="text-[10px] font-bold text-white mt-1 font-mono">{stat.value}</p>
                    </div>
                ))}
            </div>
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fallback to Gemini</span>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${settings.fallbackToGemini ? 'bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]' : 'bg-slate-700'}`}
                        >
