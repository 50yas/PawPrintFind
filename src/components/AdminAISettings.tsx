                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[7px] text-slate-500 uppercase">{t('dashboard:admin.fallbackStatus')}</span>
                        </div>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-10 h-5 rounded-full relative transition-all duration-300 ${settings.fallbackToGemini ? 'bg-primary' : 'bg-slate-700'}`}
                        >
