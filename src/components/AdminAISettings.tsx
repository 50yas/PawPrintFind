...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    visionIdentification: { icon: '🔍', color: 'text-cyan-500', borderColor: 'border-l-cyan-600' },
    ...
    smartSearch: { icon: '🕵️', color: 'text-blue-400', borderColor: 'border-l-blue-500' },
...
export const AdminAISettings: React.FC = () => {
    ...
    const tasks: { id: AIModelTask; label: string }[] = [
        ...
        { id: 'visionIdentification', label: 'Vision Identification' },
        ...
        { id: 'healthAssessment', label: 'Health Assessment' },
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini ? 'READY (GEMINI)' : 'DISABLED', icon: '🛡️', glow: settings.fallbackToGemini ? 'neon-glow-cyan' : '' },
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-4 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Failover to Gemini</span>
                            <span className="text-[8px] text-slate-500 font-mono uppercase">Resilience Mode</span>
                        </div>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-12 h-6 rounded-full relative transition-all duration-500 ${settings.fallbackToGemini ? 'bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.5)]' : 'bg-slate-700'}`}
                        >
                            <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-500 ${settings.fallbackToGemini ? 'left-7 shadow-lg' : 'left-1'}`}></div>
                        </button>
                    </div>
                </div>
