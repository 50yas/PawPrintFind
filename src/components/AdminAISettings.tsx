        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini !== false ? 'ENABLED' : 'DISABLED', icon: '🛡️', glow: settings.fallbackToGemini !== false ? 'neon-glow-teal' : '' },
                ].map((stat, i) => (
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl border transition-all duration-300 ${settings.fallbackToGemini !== false ? 'border-teal-500/30 bg-teal-500/10 text-teal-400 shadow-[0_0_15px_rgba(20,184,166,0.2)]' : 'border-white/10 bg-white/5 text-slate-500'}`}
                    >
                        <div className={`w-8 h-4 rounded-full relative transition-colors ${ settings.fallbackToGemini !== false ? 'bg-teal-500' : 'bg-slate-700'}`}>
                            <div className={`absolute top-1 w-2 h-2 bg-white rounded-full transition-all ${ settings.fallbackToGemini !== false ? 'left-5' : 'left-1'}`}></div>
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('dashboard:admin.fallbackToGemini')}</span>
                    </button>
                </div>
