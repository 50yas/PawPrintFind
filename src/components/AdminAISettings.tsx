...
export const AdminAISettings: React.FC = () => {
    ...
    const tasks: { id: AIModelTask; label: string }[] = [
        ...
        { id: 'healthAssessment', label: t('dashboard:admin.healthAssessment') || 'Health Check' },
        { id: 'blogGeneration', label: t('dashboard:admin.blogGeneration') || 'Blog Gen' },
    ];
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: 'FALLBACK STATUS', value: settings.fallbackToGemini ? 'READY' : 'DISABLED', icon: settings.fallbackToGemini ? '🛡️' : '🔘', glow: settings.fallbackToGemini ? 'neon-glow-blue' : '' },
                ].map((stat, i) => (
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 p-2 px-4 rounded-xl border border-white/10">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-2 cursor-pointer">
                            <input
                                type="checkbox"
                            ...
                            />
                            {t('dashboard:admin.fallbackToGemini') || 'Fallback to Gemini 2.0 Flash'}
                        </label>
                    </div>
                </div>
