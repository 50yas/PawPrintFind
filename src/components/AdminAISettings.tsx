...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    visionIdentification: { icon: '🆔', color: 'text-cyan-300', borderColor: 'border-l-cyan-400' },
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer" htmlFor="fallback-toggle">
                        ...
                        >
                            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform duration-300 ${settings.fallbackToGemini ? 'translate-x-6' : 'translate-x-1'}`}></div>
                        </button>
                    </div>
                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <datalist id={`models-${task.id}`}>
                                        {settings.provider === 'google' ? (
                                            ...
                                            <>
                                                {/* Recommended free models based on task */}
                                                {(task.id === 'vision' || task.id === 'visionIdentification') && (
                                                    <option value="nvidia/nemotron-nano-12b-v2-vl:free">⭐ nemotron-nano-12b-vl (Best for Vision)</option>
                                                )}
                                                {(task.id === 'blogGeneration') && (
                                                    <option value="qwen/qwen-2.5-coder-32b-instruct:free">⭐ qwen-2.5-coder-32b (Best for Coding/Logic)</option>
                                                )}
                                                <option value="qwen/qwen-2.5-72b-instruct:free">⭐ qwen-2.5-72b (Best General Purpose)</option>
