...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    healthAssessment: { icon: '🩺', color: 'text-indigo-400', borderColor: 'border-l-indigo-500' },
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-28">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    ...
                    { label: t('dashboard:admin.fallbackStatus'), value: settings.fallbackToGemini ? 'ENABLED' : 'DISABLED', icon: '🛡️', glow: settings.fallbackToGemini ? 'neon-glow-blue' : '' },
            ...
            <GlassCard className="p-6 md:p-8 border-white/10 bg-black/40 scan-hover">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('dashboard:admin.fallbackToGemini')}</label>
                        <button
                            onClick={() => setSettings({ ...settings, fallbackToGemini: !settings.fallbackToGemini })}
                            className={`w-10 h-5 rounded-full transition-all duration-300 relative ${settings.fallbackToGemini ? 'bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]' : 'bg-white/10'}`}
                        >
