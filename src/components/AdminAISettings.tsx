...
const TASK_META: Record<AIModelTask, { icon: string; color: string; borderColor: string }> = {
    ...
    smartSearch: { icon: '🔍', color: 'text-orange-400', borderColor: 'border-l-orange-500' },
    blogGeneration: { icon: '📝', color: 'text-indigo-400', borderColor: 'border-l-indigo-500' },
};
...
export const AdminAISettings: React.FC = () => {
    ...
    const tasks: { id: AIModelTask; label: string }[] = [
        ...
        { id: 'healthAssessment', label: t('dashboard:admin.healthAssessment') },
        { id: 'chat', label: t('dashboard:admin.neuralChat') },
        { id: 'matching', label: t('dashboard:admin.matchingProtocol') },
        { id: 'smartSearch', label: t('dashboard:admin.smartSearch') },
        { id: 'blogGeneration', label: t('dashboard:admin.blogGeneration') },
    ];
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
                    ...
                    <div className="flex items-center gap-4 p-3 rounded-2xl bg-white/5 border border-white/10">
                        <div className="flex flex-col">
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('dashboard:admin.fallbackToGemini')}</span>
                            <span className="text-[10px] text-slate-400">{t('dashboard:admin.fallbackToGeminiDesc')}</span>
                        </div>
                        <button
                        ...
                        >
                            <span className={`${settings.fallbackToGemini ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300`} />
                        </button>
                    </div>
                </div>
