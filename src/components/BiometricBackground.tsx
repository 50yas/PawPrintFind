...
// Particle Component
export const Particles = ({ color, scrollRef, dpr }: { color: string, scrollRef: React.MutableRefObject<number>, dpr: number }) => {
...
export const BiometricBackground = () => {
  ...
  return (
    <div className="fixed inset-0 z-[-1] transition-colors duration-1000 w-full h-full" style={{ backgroundColor: colors.background }}>
      ...
      >
        <PerformanceMonitor onDecline={() => setDpr(0.75)} onIncline={() => setDpr(1.5)}>
          ...
          <Float speed={2} rotationIntensity={0.8} floatIntensity={0.8}>
            <Particles color={colors.primary} scrollRef={scrollRef} dpr={dpr} />
          </Float>
        </PerformanceMonitor>
      </Canvas>
      ...
      {/* Note: Removed scroll-reactive vignette via state to avoid re-renders. 
          If vignette is critical, it should be a pure CSS opacity transition or part of Canvas. 
          For now, static vignette or CSS variable based approach would be better, but omitting for perf. 
      */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          backgroundImage: `radial-gradient(circle, transparent 40%, ${colors.background} 90%)`,
          opacity: 0.4
        }}
      />
