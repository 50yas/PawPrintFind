    ...
    if (!stats) {
        return (
            <div className="p-8 text-center bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10">
                ...
                <h3 className="text-xl font-bold text-white">{t('noAnalyticsData')}</h3>
                <p className="text-slate-400 mt-2">{t('noAnalyticsDataDesc')}</p>
                <button onClick={onClose} className="mt-6 glass-btn">{t('closeButton')}</button>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-slate-400">{t('aiAnalyticsSubtitle')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 p-6 rounded-2xl border border-indigo-500/20">
                    ...
                    <h3 className="text-4xl font-black text-white">{stats.averageScore.toFixed(1)}</h3>
                </div>
                <div className="bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-white/10">
                    <p className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-1">{t('totalPatientsAnalyzed')}</p>
                    <h3 className="text-4xl font-black text-white">{stats.analyzedCount} <span className="text-lg text-slate-400 font-normal">/ {stats.count}</span></h3>
                </div>
                <div className="bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-white/10 flex items-center justify-center">
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-bold text-white">{t('recentInsightsTitle')}</h3>
