        ...
        const updateRanking = async () => {
            ...
            if (hasFilters && results.length > 0) {
                ...
                if (!explanations[topPet.id]) {
                    try {
                        const explanation = await aiBridgeService.generateMatchExplanation(topPet, filters as unknown as Record<string, unknown>);
                    ...
                    <>
                        {viewMode === 'grid' && (
                            <div data-testid="pets-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in">
                                {rankedPets.map(pet => (
                                    <PetCard key={pet.id} variant="adoption" pet={pet} onViewDetail={handlePetView} onInquire={handleInquire} explanation={explanations[pet.id]} />
                                ))}
                            </div>
                        )}
                        ...
                        {viewMode === 'list' && (
                            <div className="space-y-4 animate-fade-in">
                                {rankedPets.map(pet => (
                                    <PetCard key={pet.id} variant="adoption" pet={pet} mode="list" onViewDetail={handlePetView} onInquire={handleInquire} explanation={explanations[pet.id]} />
                                ))}
                            </div>
                        )}
                        ...
                        {viewMode === 'carousel' && (
                            <div className="relative h-[600px] w-full flex items-center justify-center animate-fade-in">
                                ...
                                <div className="w-full max-w-4xl h-full">
                                    <PetCard variant="adoption" pet={rankedPets[carouselIndex]} onViewDetail={handlePetView} onInquire={handleInquire} explanation={explanations[rankedPets[carouselIndex].id]} />
                                </div>
