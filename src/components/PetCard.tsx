...
const AdoptionPetCard: React.FC<AdoptionPetCardProps> = ({ pet, onViewDetail, onInquire, mode = 'grid', explanation }) => {
    ...
    return (
        <GlassCard
            variant="default"
            className={`flex ${mode === 'list' ? 'flex-row h-48' : 'flex-col h-full'} border-white/10 bg-white/10 backdrop-blur-2xl overflow-hidden group shadow-2xl relative cursor-pointer hover:scale-[1.02] hover:shadow-primary/10 hover:border-primary/30 transition-all duration-300`}
            {...tapHandlers}
        >
