...
const AdoptionPetCard: React.FC<AdoptionPetCardProps> = ({ pet, onViewDetail, onInquire, mode = 'grid', explanation }) => {
    ...
    return (
        <GlassCard
            className={`flex ${mode === 'list' ? 'flex-row h-48' : 'flex-col h-full'} border-white/10 bg-white/10 backdrop-blur-2xl overflow-hidden group shadow-2xl relative cursor-pointer active:scale-95 transition-transform hover:scale-[1.02] hover:bg-white/15 transition-all duration-300`}
            {...tapHandlers}
        >
