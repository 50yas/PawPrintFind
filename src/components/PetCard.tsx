...
export type PetCardVariant = 'owner' | 'community' | 'mission' | 'shelter' | 'adoption';
...
interface AdoptionPetCardProps extends BasePetCardProps {
    variant: 'adoption';
...
export type PetCardProps = OwnerPetCardProps | CommunityPetCardProps | MissionPetCardProps | ShelterPetCardProps | AdoptionPetCardProps;
...
const AdoptionPetCard: React.FC<AdoptionPetCardProps> = ({ pet, onViewDetail, onInquire, mode = 'grid', explanation }) => {
    ...
    return (
        <GlassCard
            variant="default"
            className={`flex ${mode === 'list' ? 'flex-row h-48' : 'flex-col h-full'} border-white/10 bg-white/10 backdrop-blur-2xl overflow-hidden group shadow-2xl relative cursor-pointer hover:scale-[1.02] hover:bg-surface-container transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 active:scale-95`}
            {...tapHandlers}
        >
