...
describe('VetDashboard Component', () => {
  ...
  it('disables Upgrade to Pro for unverified users', () => {
    render(<VetDashboard {...defaultProps} user={mockFreeUser} />);
    const upgradeBtn = screen.getByText('Verify to Unlock PRO', { exact: false }).closest('button');
  ...
  it('enables Upgrade to Pro for approved users', () => {
    ...
    const upgradeBtn = screen.getByText('Upgrade PRO', { exact: false }).closest('button');
  ...
  it('shows Pro Active status for Pro users', () => {
    ...
    expect(screen.getByText('Pro Active', { exact: false })).toBeInTheDocument();
  });
});
