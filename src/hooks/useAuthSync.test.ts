...
describe('useAuthSync Hook', () => {
    ...
    it('syncs profile and redirects when logged in from home view', async () => {
    ...
    it('does not redirect if not on home page and already logged in', async () => {
        ...
        // 1. Initial login (redirects because wasLoggedOut is true)
        await act(async () => {
            await onAuthStateChangedCallback(mockFbUser);
        });
        ...
        // 2. Subsequent profile update (no redirect because wasLoggedOut is false and currentView is 'find')
        await act(async () => {
            await onAuthStateChangedCallback(mockFbUser);
        });
