...
describe('useAuthSync Hook', () => {
    ...
    it('does not redirect if not on home page', async () => {
        ...
        // First login triggers wasLoggedOut = true, which does trigger redirect.
        await act(async () => {
            await onAuthStateChangedCallback(mockFbUser);
        });

        // Clear mock calls to verify subsequent sync doesn't redirect
        mockSetCurrentView.mockClear();

        // Trigger onAuthStateChanged again representing subsequent sync on another page
        await act(async () => {
            await onAuthStateChangedCallback(mockFbUser);
        });
