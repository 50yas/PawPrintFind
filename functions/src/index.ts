...
async function resolveAIConfig(task: string) {
    try {
        ...
        if (doc.exists) {
            ...
            // Task-specific default free models for OpenRouter if mapping is missing
            ...
            const model = data?.modelMapping?.[task] || (provider === 'google' ? 'gemini-2.0-flash' : (defaultModels[task] || 'qwen/qwen-2.5-72b-instruct:free'));
...
async function callAI(
    ...
    if (provider === 'openrouter') {
        ...
        if (contents.parts) {
            // Check if it's a multi-turn conversation or multi-part single message
            if (contents.parts[0].role) {
        ...
        try {
            ...
            if (fallbackToGemini) {
                console.warn(`[AI Fallback] OpenRouter failed for ${featureName}. Falling back to Gemini...`, error);
...
async function callGeminiAI(
    ...
    try {
        ...
        return {
            success: true,
            text,
            parsed: config.responseMimeType === 'application/json' ? JSON.parse(text?.trim() || "{}") : undefined,
