...
/**
 * Transforms Gemini-formatted contents to OpenRouter messages.
 */
    ...
    try {
        if (provider === 'openrouter') {
            try {
                ...
                console.warn(`OpenRouter failed for ${featureName}, fallbackToGemini=${fallbackToGemini}:`, error.message);
                if (fallbackToGemini) {
                    console.info(`Falling back to Gemini for ${featureName}...`);
                    // Use Gemini 2.0 Flash as the standard reliable fallback
...
export const callOpenRouter = onCall({
    ...
    try {
        const result = await callOpenRouterAI(request.auth.uid, model, messages, config, task, openRouterApiKey.value());
        return { success: true, ...result };
    } catch (error: any) {
        console.error(`OpenRouter Error [${task}]:`, error);
        return { success: false, error: error.message || "OpenRouter call failed." };
    }
});
