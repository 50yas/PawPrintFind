...
export async function callAI(
    ...
    try {
        ...
        // Centralized JSON parsing for structured tasks
        if (config.responseMimeType === 'application/json' && result.text) {
            result.parsed = parseAIJSON(result.text);
        }

        return result;

    } catch (error: any) {
        console.error(`AI Error [${featureName}] using ${provider}:`, error);
        ...
        if (provider !== 'google' && fallbackToGemini) {
            ...
            try {
                ...
                if (config.responseMimeType === 'application/json' && fallbackResult.text) {
                    fallbackResult.parsed = parseAIJSON(fallbackResult.text);
                }
