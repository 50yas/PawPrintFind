    ...
    try {
        ...
        if (doc.exists) {
            ...
            const defaultModel = provider === 'google'
                ? 'gemini-2.0-flash'
                : (task === 'vision' || task === 'visionIdentification'
...
async function callAI(
    ...
    try {
        if (provider === 'openrouter') {
            ...
            if (contents.parts && !Array.isArray(contents)) {
                messages = contents.parts.map((p: any) => {
                    if (p.text) return { role: 'user', content: p.text };
                    if (p.inlineData) {
                        return {
                            role: 'user',
                            content: [
                    ...
                    if (p.role) {
                        return { role: p.role === 'model' ? 'assistant' : 'user', content: p.parts[0].text };
                    }
                    return null;
                }).filter(Boolean);

                // If multiple parts are just text, merge them for simpler models
                if (messages.length > 1 && messages.every((m: any) => typeof m.content === 'string')) {
                    messages = [{ role: 'user', content: messages.map((m: any) => m.content).join("\n\n") }];
                }
            }
        ...
        console.error(`Primary AI Provider [${provider}] failed for ${featureName}:`, error);
    ...
    if (result && result.success && result.text && config.responseMimeType === 'application/json') {
