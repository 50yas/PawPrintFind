 ...
 * Also handles cases where a direct model name is passed as a task.
 */
async function resolveAIConfig(task: string) {
    // If task looks like a direct model ID (e.g., gemini-1.5-pro or openai/gpt-4o)
    const isModelId = task.includes('-') || task.includes('/');
    ...
    try {
        ...
        if (doc.exists) {
            ...
            const fallbackToGemini = data?.fallbackToGemini !== undefined ? data.fallbackToGemini : true;
            ...
            if (!model) {
                if (isModelId) {
                    ...
                    model = (provider === 'google' ? 'gemini-2.0-flash' : 'qwen/qwen-2.5-72b-instruct:free');
                }
            }
    ...
    return { provider: 'google', model: isModelId ? task : 'gemini-2.0-flash', fallbackToGemini: true };
}

/**
 * Transforms Gemini-formatted multi-turn history or part arrays into OpenAI/OpenRouter messages.
 */
function transformGeminiToOpenRouter(contents: any, config: any = {}): any[] {
    // 1. If it's already an array of messages (OpenAI format), return it
    ...
    // 2. Handle Gemini contents array (multi-turn history)
    // Format: [ { role: 'user', parts: [...] }, { role: 'model', parts: [...] } ]
    if (Array.isArray(contents)) {
        messages = contents.map(turn => ({
            role: turn.role === 'model' ? 'assistant' : turn.role,
            content: Array.isArray(turn.parts)
                ? turn.parts.map((p: any) => p.text || "").join("")
                : (turn.text || "")
        }));
    }
    // 3. Handle Gemini single-turn parts object (new SDK style)
    // Format: { parts: [ { text: '...' }, { inlineData: ... } ] }
    else if (contents && contents.parts && Array.isArray(contents.parts)) {
        const hasRoles = contents.parts.some((p: any) => p.role);

        if (hasRoles) {
            // It's actually history wrapped in a 'parts' object
            messages = contents.parts.map((p: any) => ({
                role: p.role === 'model' ? 'assistant' : p.role,
                content: Array.isArray(p.parts)
                    ? p.parts.map((part: any) => part.text || "").join("")
                    : (p.text || "")
            }));
        } else {
            // It's a single turn with multiple parts (text, vision)
    ...
    // 4. Inject System Instruction if provided separately in config
...
export async function callAI(
    ...
    if (provider === 'openrouter') {
        ...
        try {
            ...
            if (fallbackToGemini) {
                console.warn(`[AI Fallback] OpenRouter failed for ${featureName}, falling back to Gemini:`, error.message);
...
async function callGeminiAI(
    ...
    try {
        ...
        const text = (candidate?.content?.parts as any)?.find((p: any) => p.text)?.text || "";
        const inlineData = (candidate?.content?.parts as any)?.find((p: any) => p.inlineData)?.inlineData;
...
export const callGemini = onCall({
    ...
    if (!request.auth) throw new HttpsError("unauthenticated", "Auth required.");
    const { task, model, contents, config } = request.data;

    // Backward compatibility: If 'model' is provided but not 'task', use 'model' as 'task'
    const targetTask = task || model;

    if (!targetTask) throw new HttpsError("invalid-argument", "Task or Model identifier required.");
