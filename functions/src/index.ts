...
async function resolveAIConfig(task: string) {
    try {
        ...
        if (doc.exists) {
            ...
            const fallbackToGemini = data?.fallbackToGemini !== false; // Default to true if not explicitly false
...
async function callAI(
    ...
    if (provider === 'openrouter') {
        ...
        if (contents && contents.parts) {
            ...
            if (imgPart) {
                messages = [{
                    role: 'user',
                    content: [
                        { type: 'text', text: (contents.parts.find((p: any) => p.text) as any)?.text || "" },
        ...
        try {
            ...
            if (fallbackToGemini) {
                console.warn(`[Fallback] OpenRouter failed for ${featureName}, falling back to Gemini. Error:`, error);
