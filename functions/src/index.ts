...
/**
 * Converts Gemini-formatted contents to OpenRouter/OpenAI message format.
 */
function transformGeminiToOpenRouter(contents: any) {
    if (Array.isArray(contents)) return contents; // Already in message format
...
/**
 * Universal AI Caller that routes to the active provider with optional fallback.
 */
async function callAI(
    ...
    if (provider === 'openrouter') {
        ...
        try {
            ...
            if (fallbackToGemini) {
                console.warn(`OpenRouter failed for ${featureName}, falling back to Gemini 2.0 Flash:`, error.message);
                // When falling back, we ensure contents is in Gemini format (parts array)
                const geminiContents = contents.parts ? contents : { parts: [{ text: messages[0]?.content || "" }] };
