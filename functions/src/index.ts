...
async function resolveAIConfig(task: string) {
    try {
        ...
        if (doc.exists) {
            ...
            // Allow direct model names to bypass mapping if they contain slashes or dashes
            const isDirectModel = task.includes('/') || task.includes('-');
            const model = isDirectModel ? task : (data?.modelMapping?.[task] || (provider === 'google' ? 'gemini-2.0-flash' : 'qwen/qwen-2.5-72b-instruct:free'));
...
/**
 * Converts Gemini contents/parts format to OpenRouter messages.
 */
function transformGeminiToOpenRouter(contents: any) {
    if (!contents) return [];
    if (Array.isArray(contents)) {
        // If it's already an array, assume it's standard history or messages
        return contents.map((turn: any) => ({
            role: turn.role === 'model' ? 'assistant' : turn.role || 'user',
...
async function callAI(
    ...
    try {
        ...
        console.warn(`[AI CALL] Provider ${provider} failed for ${featureName}. FallbackEnabled: ${fallbackToGemini}`, error);

        if (fallbackToGemini && provider !== 'google') {
            console.log(`[AI FALLBACK] Attempting recovery with Gemini 2.0 Flash for ${featureName}`);
            try {
                return await callGeminiAI(userId, featureName, 'gemini-2.0-flash', contents, config, geminiApiKey.value());
            } catch (fallbackError: any) {
                console.error(`[AI CRITICAL] Fallback failed for ${featureName}:`, fallbackError);
                throw fallbackError;
            }
        }
        throw error;
