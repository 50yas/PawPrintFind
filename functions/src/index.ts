    ...
    try {
        ...
        if (doc.exists) {
            ...
            const model = data?.modelMapping?.[task] ||
                          (provider === 'google' ? 'gemini-2.0-flash' : (defaultModels[task] || 'qwen/qwen-2.5-72b-instruct:free'));
...
async function callAI(
    ...
    try {
        if (provider === 'openrouter') {
            ...
            // Standardize JSON response format for OpenRouter/OpenAI
            ...
            if (contents.parts && !Array.isArray(contents)) {
                // Handle complex multimodal parts or simple text
            ...
            try {
                ...
                if (fallbackToGemini) {
                    console.warn(`[AI Fallback] OpenRouter failed for ${featureName}, attempting Gemini recovery...`, error);
        ...
        // Final catch-all for any provider errors
        throw error;
    }
}
