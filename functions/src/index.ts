...
async function resolveAIConfig(task: string) {
    try {
        ...
        if (doc.exists) {
            const data = doc.data();
            const provider = data?.provider || data?.activeProvider || 'openrouter'; // Default to openrouter for free models
            ...
            const defaultOpenRouterModel = taskDefaults[task] || 'qwen/qwen-2.5-72b-instruct:free';
            const model = data?.modelMapping?.[task] || (provider === 'google' ? 'gemini-2.0-flash' : defaultOpenRouterModel);
            return { provider, model };
        }
    } catch (e) {
        console.warn("Failed to resolve AI config, defaulting to OpenRouter free models:", e);
    }
